...
```bash
python3 longbridge_tax.py --year 2025 -o out_<账号>/ --rate 0.90322 --fx-rate USD=7.10
```

`-o` 是**按账号区分的基础目录**；脚本会自动把年份作为子目录嵌套，输出落在
`out_<账号>/<年份>/`（例如 `out_H10764613_M/2025/`）。每个账号用不同的 `-o`，不同账号、
不同年份就不会互相覆盖。CLI 没有账户选择参数，用的是 token 绑定的那个账户，而对账单里
**不含账号**，所以请自己用账号给 `-o` 目录命名，并确认 CLI 当前登录的就是该账户。

`--rate` 保留为 HKD→RMB 年末中间价的简写。如果结单里同时有多种货币，用
`--fx-rate 币种=汇率` 逐个传入，例如 `--fx-rate HKD=0.90322 --fx-rate USD=7.10`。
脚本会先按货币分桶；只有传了对应汇率的币种，才会填写人民币和应纳税额列。

如果 `--year 2025` 且没有传任何汇率，脚本会默认使用 2025-12-31 人行/中国外汇交易中心中间价：
`HKD=0.90322`、`USD=7.0288`。显式传入 `--rate` 或 `--fx-rate` 时，以你传入的值为准。
...
| `longbridge_<年>_税务汇总.csv` | 按货币分组的税务汇总：财产转让/分红/利息 + 应纳税额 |

脚本会打印已实现盈亏合计、分红、利息，方便核对。

## 已实现盈亏怎么算

按标的和货币分别做移动加权平均（长桥现金账户，主要做多）。`clear_amount` 带符号且已扣费；已实现只在**卖出**时
